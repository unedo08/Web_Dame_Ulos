<template>
  <div class="pm-page">
    <title>Setting - Jenis Pengiriman</title>
    <h1 class="pm-title">Jenis Pengiriman</h1>
      ...
      <table class="pm-table">
        <thead>
          <tr>
            ...
            <th class="pm-th">Nama Pengiriman</th>
        ...
        <tbody v-if="paginatedData.length > 0">
          <tr v-for="(item, index) in paginatedData" :key="item.id" class="pm-tr">
            ...
            <td class="pm-td">{{ item.jenispengiriman_nama }}</td>
            <td class="pm-td">{{ formatDate(getDisplayDate(item)) }}</td>
            <td class="pm-td pm-td-status">
              <span :class="['pm-chip', statusChipClass(item.jenispengiriman_status)]">
                {{ statusLabel(item.jenispengiriman_status) }}
              </span>
            </td>
      ...
      <div class="pm-modal">
        <div class="pm-modal-header">
          <h2 class="pm-modal-title">
            {{ isEditMode ? "Edit Jenis Pengiriman" : "Tambah Jenis Pengiriman" }}
          </h2>
        ...
        <div class="pm-modal-body">
          <label class="pm-label">
            Nama Pengiriman <span class="pm-required">*</span>
          </label>
          <input
            v-model="form.jenispengiriman_nama"
            type="text"
          ...
          <div class="pm-toggle-box">
            <div class="pm-toggle-info">
              <span class="pm-toggle-title">Status jenis pengiriman</span>
              <span class="pm-toggle-desc">
                {{ form.jenispengiriman_status == 1 ? "Akan dapat digunakan dalam transaksi" : "Tidak dapat digunakan dalam transaksi" }}
              </span>
            </div>
            <label class="pm-switch">
              <input
                type="checkbox"
                :checked="form.jenispengiriman_status == 1"
                @change="form.jenispengiriman_status = $event.target.checked ? 1 : 0"
              />
        ...
        <div class="pm-modal-footer">
          ...
          <button
            class="pm-btn-simpan"
            :disabled="!form.jenispengiriman_nama.trim() || isLoading"
            @click="handleSave"
          >
...
import { useJenisPengiriman } from "@/composables/useJenisPengiriman";
...
async function handleSave() {
  ...
  if (success) {
    Swal.fire({
      title: isEdit ? "Jenis Pengiriman Berhasil Diupdate" : "Jenis Pengiriman Berhasil Ditambahkan",
      icon: "success",
      timer: 1500,
    ...
    Swal.fire({
      title: "Gagal",
      text: isEdit ? "Gagal mengupdate jenis pengiriman." : "Gagal menambahkan jenis pengiriman.",
      icon: "error",
...
async function confirmDelete(item) {
  ...
  const result = await Swal.fire({
    ...
    text: `"${item.jenispengiriman_nama}" akan dihapus.`,
    icon: "warning",
  ...
  if (result.isConfirmed) {
    ...
    Swal.fire({ title: "Jenis Pengiriman Berhasil Dihapus", icon: "success", timer: 1500, showConfirmButton: false });
  }
}
